...
def main(gpu, batch_size, alpha, beta, gamma, omega, margin, d_arch, d_rand,
  ...
  #locals().iteritems(): python3 ではitem()
  ...
  for folder in folders:
    ...
    for cfile in files:
      ...
      if positions.shape[0] >= min_steps:
        ...
        #モーションセット分キャラ名が追加　AyaAyaAya・・Malcom・・
  ...
  with tf.device("/gpu:%d" % gpu):
    #クラスの引数に渡すと，クラス内の__init__関数へと自動的に渡される
    #だから，そのクラスの__init__関数定義では同じ引数を書いておかないといけない
